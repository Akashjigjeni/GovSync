...
export interface CitizenProfile {
  id: string;
  aadhaarNumber: string; // Masked e.g. XXXX-XXXX-4819
  fullName: string;
  fatherName: string;
  dob: string;
  ...
  verifiedCredentials: {
    ...
    landRecordId?: string; // For Agriculture
...
export interface GovernmentService {
  ...
  description: string;
  benefit: string;
  requiredProfileFields: string[]; // e.g. ['fullName', 'aadhaarNumber', 'bankAccount', 'landRecordId']
...
export interface AuditLogEntry {
  ...
  action: 'CONSENT_GRANTED' | 'CONSENT_REVOKED' | 'GATEWAY_ROUTE' | 'DATA_NORMALIZED' | 'SERVICE_SUBMIT' | 'OFFICER_APPROVED' | 'OFFICER_REJECTED' | 'PROFILE_UPDATE' | 'CITIZEN_LOGIN' | 'CITIZEN_REGISTER';
  serviceName?: string;
